...
action = function(host)
  ...
  local status, name, cpe, hints = osinfo.get_os_info(smb.get_windows_version(response.os) .. " (" ..response.lanmanager ..")")
  ...
  port.version = {
    name = "smb",
    name_confidence = 10,
    product = hints.vendor,
    version = hints.osgeneration,
    hostname = hostname,
    ostype = response.os,
    cpe = {response.cpe}
  }
