...
WINDOWS = {
  ...
  get_build = function(version_string)
    if not version_string:match "Server" then
      return version_string:match "%d%d%d%d?"
    else
  ...
  parse  =  function (version_string, look_at, hints)
    ...
    if not cpe then
      cpe = CPE:new(version_string)
      if cpe:make_cpe() then
        cpe = select(2, cpe:make_cpe())
      else
        cpe = false
        stdnse.debug2("Couldn't Create CPE")
      end
    end

    -- let's check if the cpe has the service pack
    -- TODO Add More Update Types
    if cpe then
      if not cpe:match "::" then
        local update_type, update_version = WINDOWS.get_update(version_string)
        if update_type and update_version then
          cpe = cpe .. "::" .. update_type .. update_version
        else
          cpe = cpe
        end
      end
    ...
    for _, edition in pairs(WINDOWS.Editions) do
      if version_string:match(edition) then
        if cpe and not cpe:match("::") then
          cpe = cpe .. "::"
        end
        if cpe then
          cpe = cpe .. ":" .. edition:lower()
        end
        name = name .. " " .. edition
        break
      end
...
CPE = {
  ...
  make_cpe = function(self)
    ...
    if string.match(self.version_string, "^Windows 5%.0") or string.match(self.version_string, "^Windows 2000") then
  ...
  encode = function(self)
    ...
    cpe = stdnse.strsplit(":", self.cpe_string:sub(6))

    for i, part in ipairs(cpe) do
      cpe[CPE.Parts[i]] = part
      cpe[i] = nil
    end

    if #cpe > 0 then return true, cpe else return false end

  end,

  decode = function(self)

    if self.cpe_table and next(self.cpe_table) then
...
local db = {
  ["Microsoft"] = {
    ["Windows"] = {
      ...
      ["6.3"] = {
        name = "Windows 8.1 or Windows Server 2012",
        ["9200"] = {
          client = {
            name = "Windows 8.1",
