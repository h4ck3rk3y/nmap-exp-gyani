...
local function validate_options(options)
  ...
  for key, value in pairs(options) do
    ...
    elseif(key == "auto") then
      if (options.auto == true) then
        stdnse.debug1("Proceeding with auto authentication.")
      else
        stdnse.debug1("Auto Auth Disabled for this request.")
      end
    else
...
--- A function that if Auth is needed
-- @param response The response returned by the request function
-- @returns True or False. If true then Auth Type.
local function needs_auth(response, data)
  if data:match("Authorization") or response.status ~= 401 then
    return false
  else
    if response_contains(response, "NTLM") then
      return true, "NTLM"
    elseif response_contains(response, "Basic") then
      return true, "Basic"
    elseif response_contains(response, "Digest") then
      return true, "Digest"
    else
      return false
    end
...
--- A function that performs auth
-- @param original_response The response returned by request().
-- @param auth_type either NTLM, DIGEST or BASIC
-- @param host the target host
-- @param port the port to use.
-- @param data the data that was sent by the request function.
-- @param options, the options passed to request().
-- @return A response, either the original response(incase user:pass not speicified) or the authentication response.
local function do_auth(original_response, auth_type, host, port, data, options)
  local username = stdnse.get_script_args("http.username") or false
  local password = stdnse.get_script_args("http.password") or false
  local credentials = {}
  local c = creds_lib.Credentials:new(creds_lib.ALL_DATA, host, port)
  if not (username or password) then
    for cred in c:getCredentials(creds_lib.State.VALID) do
      table.insert(credentials, {user = cred.user, pass = cred.pass})
    end
    if not next(credentials) then
      stdnse.debug1("No creds entered and no valid creds in registry.")
      return original_response
    end
  else
    table.insert(credentials, {user=username, pass=password})
  end

  for _, cred in pairs(credentials) do
    options = options or {}
    if auth_type == "Digest" then
      ...
      options.auth = { username = cred.user, password = cred.pass, digest = true }
      options.no_cache = true
    elseif auth_type == "NTLM" then
      options.auth = { username = cred.user, password = cred.pass, ntlm = true }
      options.no_cache = true
    else
      options.auth = { username = cred.user, password = cred.pass }
      options.no_cache = true
    end
    options.auto = false
    local method, path = data:match("(%w*) ([%p%w]*) HTTP/1.1")
    local response = generic_request(host, port, method, path, options)
    if response.status ~= 401 then
      c:add(cred.user, cred.pass, creds_lib.State.VALID )
      return response
    else
      c:add(cred.user, cred.passs, creds_lib.State.EXPIRED)
    end
  end
...
local function request(host, port, data, options)
  ...
  if options.auto and needs_auth(response, data) then
    return do_auth(response, select(2, needs_auth(response, data)), host, port, data, options)
  else
    return response
  end
...
-- if the last argument "socket" is given, then the function sends the ntlm message
-- through that very socket and returns a blob for the same socket.
local function make_ntlm_socket(host, port, method, path, options, old_socket)
    ...
    local socket, partial, opts, response

    if not old_socket then
    -- tryssl uses ssl if needed. sends the type 1 message.
      socket, partial, opts = comm.tryssl(host, port, build_request(host, port, method, path, custom_options), { timeout = options.timeout })
      ...
      old_socket.socket:send(build_request(host, port, method, path, custom_options), {timeout = options.timeout})
      repeat
        response, partial = next_response(old_socket.socket, method, old_socket.partial)
        if not response then
    ...
    auth_blob = bin.pack("<zISSISSISSISSISSISSIICCSAAAAA",
      ...
      ntlm)
    if old_socket then
      return auth_blob, partial
    else
      return auth_blob, {socket = socket, host = host, port = port, opts = opts}
    end
end

local function make_ntlm_request(ntlm_socket, method, path, options)

  local response, partial
  local socket = ntlm_socket.socket
  options["auth"] = nil
  options.header["Connection"] = "Keep-Alive"
  socket:send(build_request(ntlm_socket.host, ntlm_socket.port, method, path, options))
  repeat
    response, partial = next_response(socket, method, partial)
    if not response then
      return http_error("There was error in receiving response of type 3 message.")
  ...
  response.ssl = ( ntlm_socket.opts == 'ssl' )
  return response
end
...
function generic_request(host, port, method, path, options)
  ...
  local auto = stdnse.get_script_args("http.autoauth") or false
  if auto then
    options.auto = true
  end
  ...
  if ntlm_auth and have_ssl then
    ...
    local auth_blob, ntlm_socket = make_ntlm_socket(host, port, method, path, options)
    custom_options.ntlmauth = auth_blob
    local response = make_ntlm_request(ntlm_socket, method, path, custom_options)

    ntlm_socket.socket:close()
    return response
  end
...
function pipeline_go(host, port, all_requests)
  ...
  -- assumption if options.auto is set then options.auth is not set.
  -- TODO : ensure above assumption is implemented in build_request()
  -- This codeblock tries to find if auto auth is required and if it is then
  -- it tries to do auth. if we get anything other than 401 we proceed with
  -- authenticated requests. This is done by changing the socket incase of ntlm
  -- and also changing the options.ntlmauth field for all requests.
  -- Incase of digest options.auth.username, options.auth.password and options.digestauth
  -- is set.
  -- Incase of basic authentication options.auth.username and options.auth.password is set.
  -- For pipelined requests auto auth is assumed to be turned off by default.
  -- Enabling autoauth for the first request enables it for all other requests unless switched off
  -- for individual requests using options.auto = false. For NTLM as the socket is replaced
  -- all pipelined queries use the NTLM socket so auto auth gets enabled for all.
  local auth_required, auth_type = needs_auth(response, request)
  local username = stdnse.get_script_args("http.username") or false
  local password = stdnse.get_script_args("http.password") or false
  local is_auto_ntlm = false
  local auto = stdnse.get_script_args("http.autoauth") or false
  local c = creds_lib.Credentials:new(creds_lib.ALL_DATA, host, port)
  local credentials = {}
  if not (username or password) then
  ...
  if (all_requests[1].options.auto or auto) and auth_required and next(credentials) then
    if auth_type == "NTLM" and have_ssl then
      for _, cred in pairs(credentials) do
        username = cred.user
        password = cred.pass
        all_requests[1].options.auth = { username = username, password = password, nltm = true }
        local ntlm_blob, ntlm_socket = make_ntlm_socket(host, port, all_requests[1].method, all_requests[1].path, all_requests[1].options)
        all_requests[1].options.ntlmauth = ntlm_blob
        local ntlm_response = make_ntlm_request(ntlm_socket, all_requests[1].method, all_requests[1].path, all_requests[1].options)
        if ntlm_response and ntlm_response.status ~= 401 then
          socket:close()
          socket = ntlm_socket.socket
          is_auto_ntlm = true
          response = ntlm_response
          for _, request in ipairs(all_requests) do
              request.options.auth = { username = username, password = password, nltm = true } -- I don't Really Need This Here. @GYANI
              request.options.ntlmauth = ntlm_blob
          end
          break
        else
          all_requests[1].options.ntlmauth = nil
          all_requests[1].options.auth = nil
        end
      end
    elseif auth_type == "Digest" and have_ssl then
      for _, cred in pairs(credentials) do
        username = cred.user
        password = cred.pass
        -- Compute the response hash
        all_requests[1].options.auth = { username = username, password = password, digest = true }
        local options_with_auth_removed = tcopy(all_requests[1].options)
        options_with_auth_removed["auth"] = nil
        local r = generic_request(host, port, all_requests[1].method, all_requests[1].path, options_with_auth_removed)
        local h = r.header['www-authenticate']
        local dmd5 = sasl.DigestMD5:new(h, username, password, all_requests[1].method, all_requests[1].path)
        local _, digest_table = dmd5:calcDigest()
        all_requests[1].options.digestauth = digest_table
        request = build_request(host, port, all_requests[1].method, all_requests[1].path, all_requests[1].options)
        socket:send(request)
        local digest_response
        digest_response, partial = next_response(socket, all_requests[1].method, partial)
        if digest_response and digest_response.status ~=401 then
          for _, request in ipairs(all_requests) do
            if request.options.auto ~= false then
              request.options.digestauth = digest_table
              request.options.auth = { username = username, password = password, digest = true }
            end
          end
          ...
          all_requests[1].options.auth = nil
          all_requests[1].options.digestauth = nil
        end
      end
    elseif auth_type == "Basic" then
      for _, cred in pairs(credentials) do
        username = cred.user
        password = cred.pass
        all_requests[1].options.auth.username = username
        all_requests[1].options.auth.password = password
        request = build_request(host, port, all_requests[1].method, all_requests[1].path, all_requests[1].options)
        socket:send(request)
        local basic_response
        basic_response, partial = next_response(socket, all_requests[1].method, partial)
        if basic_response and basic_response.status ~= 401 then
          response = basic_response
          for _, request in ipairs(all_requests) do
            if request.options.auto ~= false then
              request.options.auth = { username = username, password = password}
            end
          end
          break
        else
          all_requests[1].options.auth.username = nil
          all_requests[1].options.auth.password = nil
        end
      end
    end
  end
  ...
  while #responses < #all_requests do
    ...
    if count >= limit or not socket:get_info() then
      ...
      partial = ""
      count = 0
      if is_auto_ntlm then
        local ntlm_blob
        ntlm_blob, partial = make_ntlm_socket(host, port, all_requests[1].method, all_requests[1].path, all_requests[1].options, {socket = socket, partial = partial})
        for _, request in ipairs(all_requests) do
            request.options.ntlmauth = ntlm_blob
        end
      end
