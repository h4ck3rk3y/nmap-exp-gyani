...
action = function(host)
  ...
  _, response.os, response.cpe = osinfo.get_os_info(smb.get_windows_version(response.os) .. " (" ..response.lanmanager ..")")


  -- Build normal output.
  local output_lines = {}
  if response.os and response.lanmanager then
    add_to_output(output_lines, "OS", string.format("%s", response.os))
  else
  ...
  if response.fqdn then
    ...
    hostname = string.match(response.fqdn, "^([^.]+)%.?")
  end
  ...
  local os_class = osinfo.get_os_class(smb.get_windows_version(response.os) .. " (" ..response.lanmanager ..")")

  -- set the port verson
  local port = {}
  port.number = smb.get_port(host)
  if port.number == 445 or port.number == 139 then
    port.protocol = "tcp"
  elseif port.number == 137 then
    port.protocol = "udp"
  end
  port.service = "smb"
  port.version = {
