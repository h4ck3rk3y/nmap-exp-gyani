...
local THEME_URL = 'https://wordpress.org/themes/rss/topic'
...
local function get_latest_theme_version(theme)
  stdnse.debug1("Retrieving the latest theme version of %s",theme)
  local rssurl = THEME_URL .. theme
  local latestthemerss = http.get('wordpress.org','80',rssurl)
  local latestthemepattern = 'ver=([.0-9]*)'
  local latestthemeversion = latestthemerss.body:match(latestthemepattern)
  stdnse.debug1("Latest version:%s", latestthemeversion)
  return latestthemeversion
end  
      ...
      if version then
         ...
         if apicheck == "true"  then
           ...
           if key == "plugins" then
            ...
            latestversion = get_latest_theme_version(bfqueries[i].name)
           end 
