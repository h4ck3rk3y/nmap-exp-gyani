...
LinkExtractor = {
  ...
  -- @param html The html body to extract links from.
  -- @param options
  ...
  --- A function that checks if a link is absolute or not.
  -- @param url
  -- @return true if it is absolute, else false
  ...
  --- Validates if a link agrees with our options table.
  -- @param url
  ...
  -- processed in here.
  -- Populates the self.links table.
  parse = function(self)
    local links = {}
    local patterns = {
      '[hH][rR][eE][fF]%s*=%s*[\'"]%s*([^"^\']-)%s*[\'"]',
...
UrlQueue = {
  ...
  -- TODO: why can't we return text instead of printing.
  dump = function(self)
...
Crawler = {
  ...
  -- @return True if the url is within the host. False otherwise
  ...
  -- @param u URL that points to the resource we want to check.
  -- @return True if the url is within the domain. False otherwise
  ...
  -- @return True if the url links to a resource.
  ...
  -- can be disabled using noblacklist=true.
