...
action = function(host, port)
  ...
  if(favicon_uri) then
    ...
    answer = http.get( host, port, root .. "/" .. favicon_uri, {auto = true})
    stdnse.debug4("Using URI %s", favicon_uri)
  else
    -- Otherwise, first try parsing the home page.
    index = http.get( host, port, root .. "/" , {auto = true})
    if index.status == 200 or index.status == 503 then
      ...
      if icon then
        ...
        if (icon_host == host.ip or
          ...
          -- request the favicon
          answer = http.get( icon_host, icon_port, icon_path, {auto = true})
        else
          answer = nil
        end
      else
    ...
    if not answer or answer.status ~= 200 then
      answer = http.get( host, port, root .. "/favicon.ico", {auto = true})
