...
action = function(host, port)
  ...
  png_icon_response = http.get(host, port, PNG_ICON_QUERY, {auto = true})
  gif_icon_response = http.get(host, port, GIF_ICON_QUERY, {auto = true})
  ...
  response = http.get(host, port, STYLESHEET_QUERY, {auto = true})
  ...
  response = http.get(host, port, VENDORS_QUERY, {auto = true})
  if response.body and response.status == 200 then
