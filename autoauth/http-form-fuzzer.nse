...
local function fuzz_form(form, minlen, maxlen, host, port, path)
  ...
  if form["method"]=="post" then
    sending_function = function(data) return http.post(host, port, form_submission_path, {auto=true}, nil, data) end
  else
    sending_function = function(data) return http.get(host, port, form_submission_path.."?"..url.build_query(data), {no_cache=true, bypass_cache=true, auto=true}) end
  end
...
function action(host, port)
  ...
  for _,target in ipairs(targets) do
    ...
    if path then
      local response = http.get( host, port, path, {auto = true} )
