  ...
  for i = 1, #fingerprints, 1 do
    ...
    for j = 1, #fingerprints[i].probes, 1 do
      if fingerprints[i].probes[j].nopipeline then
        local res = http.generic_request(host, port, fingerprints[i].probes[j].method or 'GET', basepath .. fingerprints[i].probes[j].path, {auto = true})
        if res.status then
        ...
        all = http.pipeline_add(basepath .. fingerprints[i].probes[j].path, {auto = true}, all, fingerprints[i].probes[j].method or 'GET')
      end
    end
  end
