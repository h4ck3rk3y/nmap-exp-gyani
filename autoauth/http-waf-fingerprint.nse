...
naxsi = {
  ...
  intensive = function(host, port, root, responses)
    ...
    local response = http.get(host, port, root .. "?a=[", {auto = true}) -- This shouldn't trigger the rules
    local response2 = http.get(host, port, root .. "?a=[[[]]]][[[]", {auto = true}) -- This should trigger the score based rules
...
local send_requests = function(host, port, root)
  ...
  -- Normal index
  all = http.pipeline_add(root, {auto = true}, all, "GET")
